...
function Week4({ week4Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week4Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week4Items) =>
      [...week4Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week4Items);
