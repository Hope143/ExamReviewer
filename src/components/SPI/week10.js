...
function Week10({ week10Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week10Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week10Items) =>
      [...week10Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week10Items);
    ...
    <div className="px-2 py-3 px-sm-5 pt-sm-5 pb-sm-0 bg-light">
      <div>
        <div className="shuffleButtonContainer d-flex bg-light">
          ...
          <h3 className="fw-bold text-light bg-dark p-2 m-0 ms-3 rounded-2">
            {weekNo}
          </h3>
        </div>
        {Week1Handler()}
