...
function Week8({ week8Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week8Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week8Items) =>
      [...week8Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week8Items);
