...
function Week11({ week11Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week11Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week11Items) =>
      [...week11Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week11Items);
