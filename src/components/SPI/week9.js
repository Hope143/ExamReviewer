...
function Week9({ week9Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week9Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week9Items) =>
      [...week9Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week9Items);
