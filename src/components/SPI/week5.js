...
function Week5({ week5Items, weekNo }) {
  const [showAnswer, setShowAnswer] = useState(false);
  const [showAllAnswer, setShowAllAnswer] = useState(false);
  const [getButtonNo, setGetButtonNo] = useState();

  const [newWeekList, setNewWeekList] = useState([...week5Items]);

  const ShuffleHandler = (e) => {
    e.preventDefault();
    const shuffle = (week5Items) =>
      [...week5Items].sort(() => Math.random() - 0.5);
    const newWeek1 = shuffle(week5Items);
